...
const showCorrespondingHeros = () => {
  ...
  xhr.onerror = function () {
    document.getElementById("characterSection").innerHTML =
      '<h2 id="characterMainTitle">Check internet connection...</h2>';
  };
        ...
        html += `
            <div class="col-lg-4 col-sm-12 col-md-6">
              <div class="card" style="">
                <img class="card-img-top" style="width:100%; height:80%;" onclick="showDetails(${element.id})" src="${element.image.url}">
                <div class="card-body">
                    <span class="card-title" onclick="showDetails(${element.id})">${element.name}</span>
                    <span id="${element.id}" onclick="addFavourite(${element.id})" style="cursor:pointer;">Add</span>
                </div>
              </div>
            </div>    `;
...
function addFavourite(id) {
  if (!arr.includes(id) == true) {
    ...
    alert("Your hero add in favourites");
  } else {
  ...
  xhr.onload = function () {
    ...
    // IF EVERYTHING IS FINE
      ...
      output =
        ...
        "Character" +
        "</h2>" +
        '<div class="card cardC flex-md-row mb-4 box-shadow h-md-250" id="characterCard">' +
